...
namespace RdwApi
{
    public class RdwClient
    {
        ...
        public async Task<Car?> GetCarAsync(string licensePlate) =>
            (await GetAsync<Car>($"m9d7-ebf2.json?kenteken={licensePlate.Replace("-", "").ToUpper()}"))?.WithClient(this);
        ...
        public async Task<FuelInfo?> GetFuelInfoAsync(string licensePlate) =>
            await GetAsync<FuelInfo>($"8ys7-d773.json?kenteken={licensePlate.Replace("-", "").ToUpper()}");
        ...
        protected async Task<T?> GetAsync<T>(string endpoint)
        {
            ...
            return JsonSerializer.Deserialize<IEnumerable<T>>(content, _serializerOptions)!.FirstOrDefault();
        }
    }
}
